    ...
    const FilterSidebar = () => (
        <div className="space-y-8">
                    ...
                    {jobTypes.map(t => (
                        <label key={t.value} className="flex items-center gap-3 text-sm font-semibold text-slate-600 cursor-pointer hover:text-blue-600 transition-colors">
                            <input type="radio" name="type" value={t.value} checked={type === t.value}
                                onChange={() => setType(t.value)} className="w-4 h-4 text-blue-600 focus:ring-blue-500 border-slate-300" />
                            {t.label}
                        </label>
                    ))}
                    {type && (
                        <button onClick={() => setType('')} className="text-[10px] font-bold text-blue-600 hover:underline uppercase tracking-wider mt-2">Clear</button>
                    )}
                </div>
            </div>
            ...
            <div className="pt-4 space-y-3">
                <button onClick={fetchJobs} className="btn-primary w-full text-sm py-3 shadow-lg shadow-blue-100">Apply Filters</button>
                <button onClick={() => { setType(''); setCategory(''); setMinSalary(''); setMaxSalary(''); }}
                    className="btn-secondary w-full text-sm py-3 font-bold">Reset</button>
            </div>
        </div>
    );
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
                        <div>
                            ...
                            {q && <p className="text-sm font-semibold text-slate-400 mt-2">Results for "<span className="text-blue-600">{q}</span>"</p>}
                        </div>
                    ...
                    {loading ? (
                        ...
                        <div className="text-center py-24 glass-card">
                            ...
                            <button onClick={() => { setQ(''); setLocation(''); setType(''); setCategory(''); navigate('/search'); }}
